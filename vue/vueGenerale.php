...
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="../ressources/css/general.css?v=<?= time() ?>">
    <title><?php
        /**
         * @var string $titre
         */
...
<main>
    ...
    $messagesFlash = Session::getInstance()->recupererMessagesFlash();
    if (!empty($messagesFlash)): ?>
        <div id="messages-flash">
            <?php foreach ($messagesFlash as $type => $messages): ?>
                <?php foreach ($messages as $index => $message): ?>
                    <div class="card <?= htmlspecialchars($type) ?>" id="message-<?= $type . $index ?>">
                        ...
                        <div class="message-text-container">
                            ...
                            <p class="sub-text"><?= $type === 'success' ? 'Action réussie' : 'Erreur rencontrée' ?></p>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15" class="cross-icon" onclick="removeMessage('message-<?= $type . $index ?>')">
                            <path
                                    fill="currentColor"
